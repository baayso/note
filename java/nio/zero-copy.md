...
1.1 JVM向操作系统发起`read()`系统调用，由用户空间切换到内核空间。
1.2 操作系统向硬件发出读取数据的请求。
1.3 通过`DMA`(直接内存访问)方式将数据读取到内核空间的缓冲区当中。第一次数据拷贝。
1.4 将内核空间缓冲区的数据原样拷贝到用户空间的缓冲区当中。第二次数据拷贝。由内核空间切换到用户空间。
1.5 进行业务逻辑处理...
1.6 JVM向操作系统发起`write()`系统调用，将用户空间缓冲区的数据拷贝到内核空间缓冲区当中。第三次数据拷贝。由用户空间切换到内核空间。
1.7 操作系统将内核空间缓冲区的数据写回硬件。第四次数据拷贝。
1.8 硬件通知操作系统完成数据写入。
1.9 `write()`系统调用完成并返回结果，由内核空间切换到用户空间。
